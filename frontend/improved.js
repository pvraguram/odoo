...
function updateBackendStatus(status, message) {
    ...
    const colors = {
        loading: 'bg-stone-100 text-stone-700 border-stone-200',
        connected: 'bg-green-50 text-green-700 border-green-200',
        disconnected: 'bg-red-50 text-red-700 border-red-200'
    };

    button.className = `flex items-center px-4 py-2 rounded-full shadow-sm border transition-all text-sm font-medium h-10 ${colors[status] || colors.loading}`;
...
class WorkspaceManager {
    ...
    renderProjectCard(project) {
        ...
        const categoryColors = {
            ...
            'sequence-detector': 'bg-stone-100 text-stone-700 border-stone-200',
            'flipflops': 'bg-amber-100 text-amber-800 border-amber-200'
        };
        ...
        const safeDescription = escapeHtml(project.description || 'No description');
        const safeCategory = escapeHtml(project.category.replace('-', ' '));
                        ...
                        <div class="flex items-start justify-between mb-3">
                            <div class="flex items-center space-x-2">
                                ...
                                <div>
                                    ...
                                    <span class="inline-block px-2 py-1 rounded-full text-xs font-medium border ${colorClass}">${safeCategory}</span>
                                </div>
                            </div>
        ...
        return `
                    <div class="space-y-4">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                <h4 class="text-xl font-semibold text-gray-900">${safeName}</h4>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div class="bg-gray-50 p-3 rounded-lg">
                                ...
                                <p class="text-gray-900">${safeCategory}</p>
                            </div>
                            <div class="bg-gray-50 p-3 rounded-lg">
...
let workingTimeStartTime = Date.now();
let workingTimeInterval;

function updateWorkingTime() {
    const now = Date.now();
    const elapsed = now - workingTimeStartTime;
    ...
    document.getElementById('workingTime').textContent = formattedTime;
    
    // Save the elapsed time to localStorage to persist across page reloads
    localStorage.setItem('logiclab_working_time', elapsed.toString());
}

function startWorkingTimeCounter() {
    // Check if we have a saved start time
    const savedTime = localStorage.getItem('logiclab_working_time');
    if (savedTime) {
        workingTimeStartTime = Date.now() - parseInt(savedTime);
    }
    
    // Clear any existing interval
    if (workingTimeInterval) {
...
document.addEventListener('DOMContentLoaded', startWorkingTimeCounter);
...
function showError(message) {
    ...
    errorDiv.innerHTML = `
                <div class="flex items-center">
                    ...
                    <span class="text-red-700">${escapeHtml(message)}</span>
                </div>
            `;
    return errorDiv;
}
...
document.getElementById('generateTruthTable').addEventListener('click', async () => {
    ...
    try {
        ...
        // Display truth table
        const resultsDiv = document.getElementById('truthTableResults');
        ...
        let tableHTML = `
                    <div class="flex justify-between items-center mb-4">
                        <h4 class="text-lg font-semibold text-gray-900">Truth Table for: ${escapeHtml(expression)}</h4>
        ...
        result.variables.forEach(variable => {
            tableHTML += `<th class="border border-gray-300 px-4 py-2 text-center font-bold text-blue-900">${escapeHtml(variable)}</th>`;
        });
        ...
        kmapCard.innerHTML = `
                ...
                `<div class="bg-blue-50 p-4 rounded-lg">
                                ...
                                <div class="grid grid-cols-${result.kmap[0].length} gap-1 max-w-md mx-auto">
                                    ${result.kmap.map(row =>
                    row.map(cell =>
                        `<div class="border border-blue-300 p-3 text-center font-mono font-bold bg-white text-blue-900">${cell}</div>`
                    ).join('')
                ).join('')}
                                </div>
                            </div>` :
                '<div class="text-red-600 bg-red-50 p-4 rounded-lg">K-Map grid not available for expressions with more than 3 variables</div>'
                        ...
                        <div class="bg-white p-4 rounded-lg mt-4 border flex justify-center">
                            <img id="kmapImage" src="${imageUrl}" alt="K-Map visualization" class="max-w-full h-auto">
                        </div>
                        <div class="bg-green-50 p-4 rounded-lg mt-4">
                            <h5 class="font-medium text-green-700 mb-2">Simplified Expression:</h5>
                            <p class="text-lg text-blue-900 font-mono bg-white p-3 rounded border">${escapeHtml(result.simplified || 'N/A')}</p>
                        </div>
                    </div>
                `;
        ...
        circuitCard.innerHTML = `
                    ...
                    <div class="space-y-4">
                        <div class="bg-gray-50 p-4 rounded-lg border border-gray-200">
                            <pre class="whitespace-pre-wrap text-sm font-mono text-pink-900 font-bold">${escapeHtml(result.circuit_text)}</pre>
                        </div>
                        <div class="text-sm text-gray-600 mt-4">
                            <p><strong>Input Variables:</strong> ${escapeHtml(result.variables.join(', '))}</p>
                            <p class="mt-2"><strong>Total Gates:</strong> ${result.gate_count}</p>
                            <p class="mt-2"><strong>Expression:</strong> ${escapeHtml(result.expression)}</p>
                        </div>
                    </div>
                `;
            ...
            container.innerHTML = `
                ...
                <div class="bg-white border rounded-lg p-4 shadow">
                    ...
                    <div class="flex justify-center">
                        <img src="${data.step5_circuit_image}" class="max-w-lg border rounded-lg">
                    </div>
                </div>
            `;
        } catch (error) {
